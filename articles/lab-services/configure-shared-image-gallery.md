...
- After uploading an image to a shared image gallery, ensure the image version of a specific image shows 'Succeeded' under **Replication Status**. You can do this by selecting the image and going to the image versions blade for that image on the shared image gallery tab. Unless succeeded, the image will not show up under the lab. 
